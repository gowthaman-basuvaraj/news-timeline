...
// Look for a special classname
                if ($this->classNameMatch($parentNode, '/(comment|meta|footer|footnote)/')) {
                    $readability->value -= 50;
                } else if ($this->classNameMatch($parentNode, '/((^|\s)(post|hentry|entry[-]?(content|text|body)?|article[-]?(content|text|body)?)(\s|$))/')) {
...
// Assignment from index for performance. See http://www.peachpit.com/articles/article.aspx?p=31567&seqNum=5 
        ...
        if ($topDiv == null) {
            ...
            $topDiv->setAttribute("status", "failed");
        } else {
...
//$topDiv = clean($topDiv, 'h2');
            ...
            $topDiv = $this->clean($topDiv, 'script');
        }
    ...
    private function classNameMatch($node, $pattern) {
    ...
    private function classNameHas($node, $classNames) {
        if (!$node->hasAttribute('class'))
            return false;
        $class = $node->attributes->getNamedItem('class')->nodeValue;
        $class = explode(' ', $class);
    ...
    private function cleanStyles($node) {
    ...
    private function killDivs($node) {
...
// or other ominous signs :
    ...
    private function killBreaks($node) {
        $pattern = '!(<br\s*/?>(\s|&nbsp;)*){1,}!';
    ...
    private function clean($node, $tag, $minWords=1000000) {
